...
def test_lots_of_rules(create_nacc_validator):
    ...
    assert nv.errors == {
        'oldadcid': [
            "('oldadcid', ['null value not allowed']) for if {'prevenrl': {'allowed': [1]}} then {'oldadcid': {'nullable': False}} - compatibility rule no: 0"
        ]
    }
    assert not nv.validate({'adcid': 0, 'prevenrl': 0, 'oldadcid': 1})
    assert nv.errors == {
        'oldadcid': [
            "('oldadcid', ['must be empty']) for if {'prevenrl': {'allowed': [0, 9]}} then {'oldadcid': {'nullable': True, 'filled': False}} - compatibility rule no: 1"
        ]
    }
